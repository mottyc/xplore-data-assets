    ...
    function databasesController(pfViewUtils, databasesManager, MdaDatabaseModel, Notifications) {
        ...
        self.loadEntities = function () {
            ...
            if (self.queryFilters && self.queryFilters.length > 0) {
                databasesManager.search(self.currentPage, self.querySort, self.queryFilters).then(self.postLoad);
            } else {
                databasesManager.getAll(self.currentPage).then(self.postLoad);
            }
        ...
        self.sortConfig = {
            fields: [
                { id: 'dbName', title:  'Name', sortType: 'alpha' },
                { id: 'dbNameDisplay', title:  'Display Name', sortType: 'alpha' },
                { id: 'dbTypeCd', title:  'Type', sortType: 'alpha' }
            ],
