...
angular.module('myApp')
    ...
    .directive("pfSelectSystemsModalTransclude", function ($parse) {
        'use strict';
        return {
            link: function (scope, element, attrs) {
                element.append($parse(attrs.pfSelectSystemsModalTransclude)(scope));
            }
        };
    })

    .directive('selectSystemsModal', function () {
        'use strict';
        return {
            restrict: 'A',
            scope: {
            ...
            templateUrl: 'views/directives/selection/select-systems.html',
            transclude: true,
            controller: ['$scope', '$modal', '$transclude', function ($scope, $modal, $transclude) {
                ...
                $scope.openModal = function () {
                    $modal.open({
                        controller: 'selectSystemsController',
                        controllerAs: 'ctrl',
                        resolve: {
                            content: function () {
                        ...
                        scope: $scope,
                        templateUrl: "select-systems-template.html"
                    })
                        .result.then(
                        function () {
                            $scope.close(); // closed
