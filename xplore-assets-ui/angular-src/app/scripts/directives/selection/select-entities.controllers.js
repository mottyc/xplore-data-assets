...
(function () {
    ...
    angular.module('myApp')
        .controller('selectEntitiesController', selectEntitiesController);

    selectEntitiesController.$inject = ['$scope', '$modalInstance', 'content', 'entitiesManager'];

    function selectEntitiesController($scope, $modalInstance, content, entitiesManager) {
        ...
        self.filterConfig = {
            fields: [
                { id: 'businessEntityName', title:  'Name', placeholder: 'Filter by entity name...', filterType: 'text' },
                { id: 'businessEntityNameDisplay', title:  'Display Name', placeholder: 'Filter by display name...', filterType: 'text' }
            ],
        ...
        self.itemChanged = function(event) {
            var index = self.selectedItems.indexOf(event.item.businessEntityKey);

            if ((event.item.isChecked == true) && (index < 0)) {
                self.selectedItems.push(event.item.businessEntityKey);
            }
