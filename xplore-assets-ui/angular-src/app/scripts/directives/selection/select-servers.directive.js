...
angular.module('myApp')
    ...
    .directive("pfSelectServersModalTransclude", function ($parse) {
        'use strict';
        return {
            link: function (scope, element, attrs) {
                element.append($parse(attrs.pfSelectServersModalTransclude)(scope));
            }
        };
    })

    .directive('selectServersModal', function () {
        'use strict';
        return {
            restrict: 'A',
            scope: {
            ...
            templateUrl: 'views/directives/selection/select-servers.html',
            transclude: true,
            controller: ['$scope', '$modal', '$transclude', function ($scope, $modal, $transclude) {
                ...
                $scope.openModal = function () {
                    $modal.open({
                        controller: 'selectServersController',
                        controllerAs: 'ctrl',
                        resolve: {
                            content: function () {
                        ...
                        scope: $scope,
                        templateUrl: "select-servers-template.html"
                    })
                        .result.then(
                        function () {
                            $scope.close(); // closed
